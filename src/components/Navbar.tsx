    ...
    <nav className="fixed top-0 left-0 w-full bg-card/90 backdrop-blur-sm py-4 px-6 shadow-md z-50">
      <div className="max-w-6xl mx-auto flex justify-between items-center w-full">
        ...
        {loading ? (
          ...
          <>
            <div className="hidden sm:flex items-center gap-4">
              <div className="font-semibold">
                <span className="font-normal text-gray-300">Olá, </span>
                {user.email
                  .split("@")[0]
                  .split(".")
                  .map(
              ...
              <Link href="/create">
                <AuthButton label="Nova Tarefa" variant="primary" />
              </Link>
          ...
          <div className="hidden sm:flex items-center gap-4">
            <Link href="/register">
              <AuthButton label="Registrar" variant="secondary" width="w-28" />
            </Link>
            <Link href="/login">
              <AuthButton label="Entrar" variant="primary" width="w-28" />
            </Link>
          </div>
        )}
      </div>
